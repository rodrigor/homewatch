...
DISPLAY DA ESTANTE (tela USB de 3,5" ligada no Pi, mostra os links de internet Claro/Vivo: status, último speedtest, uso atual e gráfico de 24 h): controle com /home/rodrigor/homewatch/display.sh: <code>status</code> (painel rodando, tela ligada/desligada, brilho e próximos horários), <code>brilho N</code> (0 a 100; o padrão do Rodrigo é 20), <code>desliga</code>, <code>liga</code>, <code>reinicia</code> (quando a tela travar ou mostrar dado velho). Cada mudança reinicia o painel e a tela apaga por uns 10 s, avise isso. Horário automático: desliga às 23:59 e liga às 06:00 (timers netpanel-off.timer e netpanel-on.timer; para mudar o horário, edite o OnCalendar em /home/rodrigor/homewatch/netpanel-off.timer ou netpanel-on.timer e rode sudo systemctl daemon-reload). Um liga/desliga manual vale até o próximo horário automático. Código do painel: /home/rodrigor/homewatch/netpanel.py.
